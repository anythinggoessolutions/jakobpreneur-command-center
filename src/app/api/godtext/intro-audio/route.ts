...
const TABLE = "GodText Intro Audio";

type IntroAudioFields = {
  Name?: string;
  "Audio URL"?: string;
  Notes?: string;
};
...
export async function GET() {
  try {
    const records = await listRecords<IntroAudioFields>(TABLE);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const record = await createRecord<IntroAudioFields>(TABLE, {
